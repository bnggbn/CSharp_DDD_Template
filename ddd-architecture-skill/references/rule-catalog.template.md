...
- `CQRS100`: `*Command` must be declared as `record` and nested in `*UseCase` under `application/use-cases/`.
- `CQRS101`: `*Query` must be declared as `record` and nested in `*UseCase` under `application/use-cases/`.
